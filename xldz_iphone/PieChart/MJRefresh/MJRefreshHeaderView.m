	...
	switch (state) {
		case MJRefreshStatePulling: // 松开可立即刷新
        {
            ...
            [UIView animateWithDuration:MJRefreshAnimationDuration animations:^{
                ...
                if (self.multiColumnView) {
                    self.multiColumnView.tblView.contentInset = inset;
                    self.multiColumnView.headerTblView.contentInset = inset;
                } else {
		...
		case MJRefreshStateNormal: // 下拉可以刷新
        {
            ...
            [UIView animateWithDuration:MJRefreshAnimationDuration animations:^{
                ...
                if (self.multiColumnView) {
                    self.multiColumnView.headerTblView.delegate = nil;
                    self.multiColumnView.tblView.delegate = nil;
                    self.multiColumnView.headerTblView.contentInset = inset;
                    self.multiColumnView.tblView.contentInset = inset;
                } else {
                    _scrollView.contentInset = inset;
                ...
                if (finished && self.multiColumnView) {
                    self.multiColumnView.headerTblView.delegate = self.multiColumnView;
                    self.multiColumnView.tblView.delegate = self.multiColumnView;
                }
            }];
		...
		case MJRefreshStateRefreshing: // 正在刷新中
        {
            ...
            [UIView animateWithDuration:MJRefreshAnimationDuration animations:^{
                ...
                if (self.multiColumnView) {
                    self.multiColumnView.tblView.contentInset = inset;
                    self.multiColumnView.tblView.contentOffset = CGPointMake(0, - _scrollViewInitInset.top - MJRefreshViewHeight);
                    self.multiColumnView.headerTblView.contentInset = inset;
                    self.multiColumnView.headerTblView.contentOffset = CGPointMake(0, - _scrollViewInitInset.top - MJRefreshViewHeight);
                } else {
                    _scrollView.contentInset = inset;
                    // 2.设置滚动位置
