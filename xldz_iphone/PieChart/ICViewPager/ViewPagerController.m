...
#define kTabsViewBackgroundColor [UIColor blackColor]
...
//    }
    if (!normalBg) {
        normalBg = [UIImage imageNamed:@"tab_bar_bg"];
    }
    if (!pressedBg) {
        pressedBg = [[UIImage imageNamed:@"tab_bar_selected_bg"] resizableImageWithCapInsets:UIEdgeInsetsMake(13, 18, 13, 18) resizingMode:UIImageResizingModeStretch];
    }
    UIImage *img = self.selected ? pressedBg : normalBg;
    [img drawInRect:rect];
}
@end
...
//    NSLog(@"topLayoutGuide: %f", topLayoutGuide);
//    NSLog(@"CGRectGetHeight(self.tabsView.frame): %f", CGRectGetHeight(self.tabsView.frame));
//    NSLog(@"CGRectGetHeight(self.tabBarController.tabBar.frame): %f", CGRectGetHeight(self.tabBarController.tabBar.frame));
