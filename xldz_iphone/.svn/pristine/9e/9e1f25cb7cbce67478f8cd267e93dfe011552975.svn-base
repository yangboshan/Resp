...
@synthesize yMeasureLine;

@synthesize origXOffset;
@synthesize origYOffset;
    ...
    for ( size_t i = 0; i < kCPTNumberOfLayers; i++ ) {
        ...
        switch ( buLayerOrder[i] ) {
            ...
            case CPTGraphLayerTypeAxisTitles:
                ...
                break;
            case CPTGraphLayerTypeMeasureLines:
                if ( self.yMeasureLine){
                    idx++;
                }
                break;
        }
...
{
    if(self.yMeasureLine!=nil){
        self.yMeasureLine.measurePoint = CGPointMake(-1,-1);
        self.yMeasureLine.relativeMeasurePoint=CGPointMake(-1,-1);
        self.yMeasureLine.hidden=YES;
        [self.yMeasureLine setNeedsDisplay];
    }
}
...
-(void)showMeasureLines
{
    ...
    if(self.yMeasureLine==nil){
        CPTMeasureLine* newMeasureLine=[[CPTMeasureLine alloc]init];
        [yMeasureLine removeFromSuperlayer];
        
        [newMeasureLine retain];
        [yMeasureLine release];
        newMeasureLine.plotArea=self;
        yMeasureLine = newMeasureLine;
        if ( yMeasureLine ) {
            [self insertSublayer:yMeasureLine atIndex:[self indexForLayerType:CPTGraphLayerTypeMeasureLines]];
        }
        [self setNeedsLayout];
    }
    self.yMeasureLine.hidden = NO;
    self.yMeasureLine.isRelativeMeasure = NO;
    [self.yMeasureLine setNeedsDisplay];
...
-(void)showMeasureLinesAtPoint:(CGPoint)point
{
    ...
    self.yMeasureLine.isRelativeMeasure = NO;
    self.yMeasureLine.measurePoint=point;
    [self.yMeasureLine setNeedsDisplay];
...
-(void)showRelativeMeasureLinesAtPoint:(CGPoint)point
{
    ...
    self.yMeasureLine.isRelativeMeasure = YES;
    self.yMeasureLine.relativeMeasurePoint=point;
    [self.yMeasureLine setNeedsDisplay];
...
-(BOOL)pointingDeviceDownEvent:(CPTNativeEvent *)event atPoint:(CGPoint)interactionPoint
{
    self.yMeasureLine.hidden = NO;
    if(self.yMeasureLine==nil){
        [self showMeasureLines];
    }
    
    
    
    if (self.yMeasureLine!=nil &&  [self.yMeasureLine pointingDeviceDownEvent:event atPoint:interactionPoint] ) {
        return YES;
    }
...
{
    if (self.yMeasureLine!=nil &&   [self.yMeasureLine pointingDeviceUpEvent:event atPoint:interactionPoint] ) {
        return YES;
    }
    return NO;
}
...
{
    if (self.yMeasureLine!=nil &&   [self.yMeasureLine pointingDeviceDraggedEvent:event atPoint:interactionPoint] ) {
        return YES;
    }
    return NO;
}
...
{
    if (self.yMeasureLine!=nil &&   [self.yMeasureLine pointingDeviceCancelledEvent:event] ) {
        return YES;
    }
    return NO;
}
