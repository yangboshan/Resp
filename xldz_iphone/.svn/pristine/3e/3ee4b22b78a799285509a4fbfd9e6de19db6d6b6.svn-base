...
@synthesize labelOnlyFirstAndLast;
...
-(void)updateAxisLabelsAtLocations:(NSSet *)locations inRange:(CPTPlotRange *)labeledRange useMajorAxisLabels:(BOOL)useMajorAxisLabels
{
    ...
    NSDecimal minLocation,maxLocation;
    ...
    for ( NSDecimalNumber *tickLocation in locations ) {
        ...
        if(i==0){
            minLocation = locationDecimal;
            maxLocation = locationDecimal;
        }
        i++;

        if(CPTDecimalGreaterThan(locationDecimal, maxLocation)) maxLocation=locationDecimal;

        if(CPTDecimalLessThan(locationDecimal, minLocation)) minLocation=locationDecimal;
    ...
    for ( NSDecimalNumber *tickLocation in locations ) {
        ...
        NSDecimal minLimit = range.minLimit;
        NSDecimal maxLimit = range.maxLimit;
        minLimit = CPTDecimalAdd(minLimit, self.labelingOrigin);
        maxLimit = CPTDecimalAdd(maxLimit, self.labelingOrigin);


        if(labelOnlyFirstAndLast==YES && !(
                NSDecimalCompare(&locationDecimal,&(minLocation))==NSOrderedSame  ||
            NSDecimalCompare(&locationDecimal,&(maxLocation))==NSOrderedSame
        ...
        if(NSDecimalCompare(&locationDecimal,&(minLocation))==NSOrderedSame){
            
            newAxisLabel.isFirstAxis=YES;
            
        }
        
        if(NSDecimalCompare(&locationDecimal,&(maxLocation))==NSOrderedSame){
            
            newAxisLabel.isLastAxis=YES;
...
//            NSLog(@"label position %f,%f",newAxisLabel.contentLayer.position.x,newAxisLabel.contentLayer.position.y);
...
-(void)updateMajorTickLabels
{
    ...
    for ( CPTAxisLabel *label in self.axisLabels ) {
        ...
        if(labelOnlyFirstAndLast==YES){
            ...
            if(label.isFirstAxis && orthogonalCoordinate==CPTCoordinateX){
                ...
                CGPoint origPoint= [self convertPoint:[self.plotSpace plotAreaViewPointForDoublePrecisionPlotPoint:plotPoint numberOfCoordinates:2] fromLayer:self.plotArea];

                
                
                
                CGPoint ptNew = CGPointMake( origPoint.x,label.contentLayer.position.y);
                
//                NSLog(@"%f,%f",self.paddingLeft,self.paddingRight);
                label.contentLayer.position= ptNew;

                
            }
            
            
//            NSLog(@"%d",orthogonalCoordinate);
            if(label.isLastAxis && orthogonalCoordinate==CPTCoordinateY){
                CGPoint ptNew = CGPointMake( label.contentLayer.position.x-label.contentLayer.frame.size.width,label.contentLayer.position.y);
                label.contentLayer.position= ptNew;
