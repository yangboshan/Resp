...
+ (void)initialize
{
    ...
    appearance.viewBackgroundColor = [UIColor blackColor];
    appearance.titleColor = [UIColor lightGrayColor];
    appearance.messageColor = [UIColor lightGrayColor];
...
- (void)showWithDelay:(int)delaySeconds{
    
    [NSTimer scheduledTimerWithTimeInterval:delaySeconds target:self selector:@selector(handleTimer:) userInfo:nil repeats:NO];
    [self show];
...
- (void)setup
{
    ...
    if (self.dismissWhenTapOutside) {
        [self whenTapped:^{
            [self dismissAnimated:YES];
        }];
        [self.containerView whenTapped:^{
