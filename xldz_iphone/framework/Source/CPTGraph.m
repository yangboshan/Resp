...
-(void)removeAllPlots{
    ...
    if(self.plots !=nil)
        ...
        while(self.plots.count>0){
            ...
            CPTPlot* plot = [self.plots objectAtIndex:0];
...
{
    [self.plotAreaFrame.plotArea showMeasureLines];
    
}

-(void)showMeasureLinesAtPoint:(CGPoint)point
{
    [self.plotAreaFrame.plotArea showMeasureLinesAtPoint:point];
    
    
    
}

-(CGPoint)convertTheSelectPoint:(CGPoint)interactionPoint{
    
    CGPoint plotAreaPoint = [self convertPoint:interactionPoint toLayer:self.plotAreaFrame.plotArea];
    
    
    NSDecimal plotInteractionPoint[2];
    [self.defaultPlotSpace plotPoint:plotInteractionPoint numberOfCoordinates:2 forPlotAreaViewPoint:plotAreaPoint];
    
//    NSLog(@"convert point=%f,%f",CPTDecimalDoubleValue(plotInteractionPoint[0]),CPTDecimalDoubleValue(plotInteractionPoint[1]));
    
//    NSLog(@"plotArea size=%f,%f",self.plotAreaFrame.plotArea.frame.size.width,
//          self.plotAreaFrame.plotArea.frame.size.height);

//    plotAreaPoint.x = plotAreaPoint.x/self.plotAreaFrame.plotArea.frame.size.width;
//        plotAreaPoint.y = plotAreaPoint.y/self.plotAreaFrame.plotArea.frame.size.height;
//    
    
    //CPTXYPlotSpace *xySpace = (CPTXYPlotSpace*)self.defaultPlotSpace;
    ...
    plotAreaPoint.x =CPTDecimalFloatValue(CPTDecimalDivide(plotInteractionPoint[0],xAxis.majorIntervalLength));
    plotAreaPoint.y =CPTDecimalFloatValue(plotInteractionPoint[1]);
    ...
    return plotAreaPoint;
    
}

-(void)hideMeasureLines
{
    [self.plotAreaFrame.plotArea hideMeasureLines];

    if(self.relativeGraph != nil){
        [self.relativeGraph.plotAreaFrame.plotArea hideMeasureLines];
    }
...
-(void)pinchScaleWithNewXRange:(CPTPlotRange*)range forScale:(CGFloat)scale notScale:(NSString*)scaleMode{
    CPTPlotRange *newXRange=range;
...
//                xySpace.yRange=oldYRange;
    ...
    if([scaleMode isEqualToString:@"FIXED"]){
        for(CPTPlot* plot in [self allPlots]){
    ...
    if([scaleMode isEqualToString:@"SCALE"]){
        ...
        for(CPTPlot* plot in [self allPlots]){
            if([plot isKindOfClass:[CPTBarPlot class]]){
                ...
                float curWidth = CPTDecimalFloatValue(barPlot.barWidth);
                curWidth=(float)(curWidth*scale);
                
                barPlot.barWidth=CPTDecimalFromCGFloat(curWidth);
            }
            ...
            if([plot isKindOfClass:[CPTTradingRangePlot class]]){
                ...
                float curWidth = (float)(barPlot.barWidth);
                curWidth=(float)(curWidth*scale);
                
                barPlot.barWidth=(curWidth);
            }
        }
    }
}
...
{
    [self.plotAreaFrame.plotArea showRelativeMeasureLinesAtPoint:point];
...
//        if(self.relativeGraph !=nil)
//            [self.relativeGraph showRelativeMeasureLinesAtPoint:interactionPoint];
