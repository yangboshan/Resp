...
- (void)layoutSubviews {
    ...
    self.pageControl.frame = CGRectMake(0, self.frame.size.height - self.pageControlY, self.frame.size.width, 20);
}
...
- (void)buildScrollView {
    ...
    [self makePanelVisibleAtIndex:_currentPageIndex];
    [_pages[_currentPageIndex] pageDidAppear];
    if ([(id)self.delegate respondsToSelector:@selector(intro:pageAppeared:withIndex:)]) {
        [self.delegate intro:self pageAppeared:_pages[_currentPageIndex] withIndex:_currentPageIndex];
    }
    ...
    self.scrollView.contentSize = CGSizeMake(contentXIndex, 0);
...
- (UIView *)viewForPage:(EAIntroPage *)page atXIndex:(CGFloat *)xIndex {
    ...
    *xIndex += self.scrollView.frame.size.width;
    ...
    if(page.customView) {
        page.customView.frame = pageView.frame;
        page.customView.autoresizingMask = pageView.autoresizingMask;
        pageView = page.customView;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (self.visiblePageIndex < _pages.count) {
        ...
        if ([(id)self.delegate respondsToSelector:@selector(intro:pageAppeared:withIndex:)]) {
            [self.delegate intro:self pageDidScrollToVisibleIndex:self.visiblePageIndex];
        }
