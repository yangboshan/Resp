...
{
    //sureone adjust the bar gap
    const CGFloat halfBarWidth       = CPTFloat(0.5) * self.barWidth-0.7f;
...
-(void)touchSelectFromExternal:(CGPoint)interactionPoint{
    ...
    if ( [theDelegate respondsToSelector:@selector(tradingRangePlot:barWasSelectedAtRecordIndex:)] ||
        ...
        if ( idx != NSNotFound ) {
            ...
            if ( [theDelegate respondsToSelector:@selector(tradingRangePlot:barWasSelectedAtRecordIndex:withEvent:)] ) {
                [theDelegate tradingRangePlot:self barWasSelectedAtRecordIndex:idx withEvent:nil];
            }
            return;
        }
    }
